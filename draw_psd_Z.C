void draw_psd_Z(){
  ...
  string AmBeRun[n]={
   ...
   c1->SetBottomMargin(0.15);
    ...
    TH2F* h = new TH2F("h","",n+1,0,n+1,35,111,127);
   ...
   gRandom->SetSeed();
   for (Int_t i=0;i<n;i++) {
      h->Fill(people[i],theZ[i]*1000.0,theZ[i]*1000.0);
      h->SetBinError(i,i,theZerr[i]*1000.0);
      h0->SetBinContent(i+1,theZ[i]*1000.0);
      h0->SetBinError(i+1,theZerr[i]*1000.0);
   }
   h->LabelsDeflate("X");
   //h->LabelsDeflate("Y");
   h->LabelsOption("v");
