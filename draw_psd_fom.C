void draw_psd_fom(){
  ...
  string AmBeRun[n]={
   ...
   c1->SetBottomMargin(0.15);
    ...
    TH2F* h = new TH2F("h","",n+1,0,n+1,35,1.25,1.60);
   ...
   gRandom->SetSeed();
   for (Int_t i=0;i<n;i++) {
      h->Fill(people[i],FOMA[i],FOMA[i]);
      h->SetBinError(i,i,FOMAerr[i]);
      h0->SetBinContent(i+1,FOMA[i]);
      h0->SetBinError(i+1,FOMAerr[i]);
   }
   h->LabelsDeflate("X");
   //h->LabelsDeflate("Y");
   h->LabelsOption("v");
